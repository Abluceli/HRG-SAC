...
class Agent:
    ...
    def unity_train(self):
        ...
        if self.use_GCN:
            adj, x, visual_state, action, dones_flag, rewards = zeros_initializer(self.env.brain_num, 6)
            ...
            for episode in range(begin_episode, max_episode):
                ...
                while True:
                    step += 1
                    for i in range(self.env.brain_num):
                        action[i] = self.models[i].choose_action(adj=adj[i], x=x[i], visual_s=visual_state[i])
    ...
    def unity_no_op(self):
        ...
        if self.use_GCN:
            adj, x, visual_state, action = zeros_initializer(self.env.brain_num, 4)
            ObsRewDone = self.env.reset()
            for i, (_adj, _x, _vs, _r, _d) in enumerate(ObsRewDone):
                adj[i] = _adj
                x[i] = _x
            ...
            for step in range(steps):
                ...
                if choose:
                    for i in range(self.env.brain_num):
                        action[i] = self.models[i].choose_action(adj=adj[i], x=x, visual_s=visual_state[i])
                else:
                    action = self.env.random_action()
                actions = {f'{brain_name}': action[i] for i, brain_name in enumerate(self.env.brain_names)}
                ObsRewDone = self.env.step(vector_action=actions)
    ...
    def unity_inference(self):
        ...
        if self.use_GCN:
            ...
            while True:
                ...
                while True:
                    for i, (_adj, _x, _vs, _r, _d) in enumerate(ObsRewDone):
                        action[i] = self.models[i].choose_action(adj=_adj, x=_x, visual_s=_vs, evaluation=True)
                    actions = {f'{brain_name}': action[i] for i, brain_name in enumerate(self.env.brain_names)}
                    ObsRewDone = self.env.step(vector_action=actions)
        else:
