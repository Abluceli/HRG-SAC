...
class ExperienceReplay(ReplayBuffer):
    ...
    def _store_op(self, data):#[adj, x, vs, a, r, adj_, x_, vs_, done]
...
class NStepWrapper:
    ...
    def add(self, *args):
        ...
        if self.use_gcn:
            # self._per_store_gcn([data for data in args])#[adj, x, vs, a, r, adj_, x_, vs_, done]
            [self._per_store_gcn(i, list(data)) for i, data in enumerate(zip(*args))]
    ...
    def _per_store_gcn(self, i, data):
        '''
        data:
            0   adj         -9
            1   x           -8
        ...
        if (q[-1][-4] != data[0]).any() or (q[-1][-3] != data[1]).any():    # 如果截断了，非常规done，把Nstep临时经验池中已存在的经验都存进去，临时经验池清空
            if len(q) == self.n:
                self._store_op(q.pop(0))
            else:
                q.clear()
        ...
        _len = len(q)
        for j in range(_len):   # 然后再存入一条最新的经验到Nstep临时经验池
            q[j][4] += data[4] * (self.gamma ** (_len - j))
            q[j][5:] = data[5:]
        q.append(data)
        if all(data[-1]):  # done or not # 如果新数据是done，就清空临时经验池
            while q:    # (1-done)会清零不正确的n-step
                self._store_op(q.pop())
        # print(len(q))
        # print(self.buffer.size)
